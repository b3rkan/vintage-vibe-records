...
<main class="vvr-main">
    <div class="vvr-container">
        ...
        <div class="empty-favorites" id="empty-message" style="display: none;">
            <h3>Henüz favori plak eklemediniz</h3>
            <p>Beğendiğiniz plakları favorilere ekleyerek kolayca erişebilirsiniz.</p>
            <a href="index.php">← Plak Koleksiyonuna Dön</a>
        </div>
    </div>
</main>
...
function displayFavorites() {
        ...
        if (plak) {
            ...
            card.innerHTML = `
                ...
                <div class="product-info">
                    <div class="product-category">${plak.kategori_adi || 'Diğer'}</div>
                    <h3 class="product-title">${plak.baslik}</h3>
                    <p class="product-artist">${plak.sanatci}</p>
                    ${plak.cikis_yili && plak.cikis_yili !== '0000' ? `<p class="product-year">📅 ${parseInt(plak.cikis_yili)}</p>` : ''}
                    <div class="product-footer">
                        <span class="product-price">${fiyat} ₺</span>
                        <a href="detay.php?id=${plak.id}" class="product-view-btn">İncele</a>
                    </div>
                </div>
            `;
    ...
    // Favoriler butonlarını aktifleştir
    initializeFavoritesDisplay();
}

function initializeFavoritesDisplay() {
...
function toggleFavoriteFull(productId) {
    ...
    localStorage.setItem('vvr_favorites', JSON.stringify(favorites));
    displayFavorites();
}
